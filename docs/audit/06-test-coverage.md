...
- R-10 (fully — R-10.1 ProcessWorker + R-10.2 ThreadWorker + R-10.3 Agent.terminate + **R-10.4 MqttBroker.stop** all bounded) via [RFC-008](../rfc/RFC-008-process-worker-lifecycle.md) + [RFC-009](../rfc/RFC-009-thread-worker-lifecycle.md) + [RFC-010](../rfc/RFC-010-broker-bounded-shutdown.md); only R-10.5 non-daemon interpreter-exit blocking remains Open (documented architectural limitation — RFC-010 helper is `daemon=True` but worker thread waiting on broker is `daemon=False`)
...
Result as of 2026-07-28 (after RFC-010 implementation):

```
377 passed, 0 failed, 0 xfailed(strict-pass), 2 xfailed  in ~34s
```

Broken down (single collected run):
- RFC-010 suite (`tests/unit/test_mqtt_broker_shutdown.py`): **53 passed** in ~2 s
- RFC-009 suite (`tests/unit/core/test_thread_worker_lifecycle.py`): **35 passed** in ~6 s (unchanged)
- RFC-008 suite (`tests/unit/core/test_process_worker_lifecycle.py`): **33 passed** in ~14 s (unchanged)
- MqttBroker reconnect / lifecycle / start / auth / callbacks: **48 + 11 + 13 + 6 + 8 = 86 passed** (2 tests in reconnect + 3 tests in lifecycle refactored to prime broker to RUNNING; RFC-005 semantics preserved)
- All other suites: **170 passed, 2 xfailed** in ~12 s (unchanged)
...
- Post-RFC-010 implementation (2026-07-28): **377 passed / 2 xfailed** (characterisation rewritten to the new contract; net +11 tests; 5 existing MqttBroker tests refactored; all other suites zero regression).
...
- **R-10 (R-10.2 + R-10.3 slices, ThreadWorker side)** — was uncovered by any bounded-lifecycle test until this phase; now covered by `tests/unit/core/test_thread_worker_lifecycle.py` (RFC-009):
  ...
  - **R-10.5** (non-daemon interpreter-exit blocking under STOP_TIMEOUT) remains **Open — documented architectural trade-off** across `ThreadWorker` / `stop()` / `Agent.terminate` / `Agent.__deactivating` docstrings and every WARNING log emitted by the timeout path (both worker and broker layers). RFC-010's helper thread is `daemon=True` (so helper alone does not block interpreter exit), but a worker thread waiting on `broker.stop()` still does.
...
| R-10 `join()` without timeout on stuck handler | **Resolved 2026-07-28 (RFC-008 + RFC-009 + RFC-010); covered by `tests/unit/core/test_process_worker_lifecycle.py` (33 tests) + `tests/unit/core/test_thread_worker_lifecycle.py` (35 tests) + `tests/unit/test_mqtt_broker_shutdown.py` (53 tests across 9 categories A–I).** R-10.1 ProcessWorker bounded escalation (RFC-008). R-10.2 ThreadWorker bounded cooperative stop with STOP_TIMEOUT retry (RFC-009). R-10.3 `Agent.terminate` bounded via observing `worker.stop()` bool + WARNING on False. **R-10.4** `MqttBroker.stop(graceful_timeout_s=5.0) -> bool` via `daemon=True` helper-thread wrapper (RFC-010): single-helper retry (same MqttBroker lifecycle → exactly one paho `disconnect + loop_stop` pair, ever); wedged → `STOP_TIMEOUT` retriable; helper abnormal exit (`BaseException`) → `STOP_FAILED` (not silently mislabeled `STOPPED`); concurrent stop callers share one helper via `_stop_complete_event.wait(t + 0.1s)` bounded margin; `disconnect` `Exception` does NOT prevent `loop_stop` (resource-leak fix); full callback fencing (`_on_message` silent drop, `_on_connect` entire body gated, `_on_disconnect` diagnostics-only). `Agent.__deactivating` observes bool + WARNING; never-raise preserved. **R-10.5** non-daemon interpreter-exit blocking under STOP_TIMEOUT remains **Open** — deliberate architectural trade-off (RFC-009 §7.13 keeps worker `daemon=False`; RFC-010 helper is `daemon=True` so helper alone doesn't block, but the worker thread waiting on broker still does). Documented in five docstrings and every WARNING log emitted by the timeout path (worker + broker + `Agent.__deactivating`). |
