...
- R-10 (fully — R-10.1 ProcessWorker + R-10.2 ThreadWorker + R-10.3 Agent.terminate + R-10.4 MqttBroker.stop + **R-10.6 MqttBroker.start** all bounded) via [RFC-008](../rfc/RFC-008-process-worker-lifecycle.md) + [RFC-009](../rfc/RFC-009-thread-worker-lifecycle.md) + [RFC-010](../rfc/RFC-010-broker-bounded-shutdown.md) + [RFC-011](../rfc/RFC-011-mqtt-broker-bounded-startup.md); only R-10.5 non-daemon interpreter-exit blocking remains Open (documented architectural limitation — RFC-010 stop helper and RFC-011 startup helper are both `daemon=True`, but worker thread waiting on broker is `daemon=False`)
...
Result as of 2026-07-29 (after RFC-011 implementation):

```
451 passed, 0 failed, 0 xfailed(strict-pass), 2 xfailed  in ~49s
```

Broken down (single collected run):
- RFC-011 suite (`tests/unit/test_mqtt_broker_startup_bounded.py`): **74 passed** in ~15 s
- RFC-010 suite (`tests/unit/test_mqtt_broker_shutdown.py`): **53 passed** in ~2 s (1 test refactored: `test_B15` state assertion updated START_FAILED → START_TIMEOUT per RFC-011 semantics)
...
- MqttBroker reconnect / lifecycle / start / auth / callbacks / empty: **48 + 11 + 13 + 6 + 8 + 3 = 89 passed** (unchanged post-RFC-011 — `startup_timeout_s=None` fallback to `self._timeout` preserves backward compat)
...
- Post-RFC-011 implementation (2026-07-29): **451 passed / 2 xfailed** (characterisation rewritten; net +9 tests: 74 new post-RFC-011 startup tests replacing 65 characterisation; 1 shutdown test refactored; all other suites zero regression).
...
- **R-10 (R-10.2 + R-10.3 slices, ThreadWorker side)** — was uncovered by any bounded-lifecycle test until this phase; now covered by `tests/unit/core/test_thread_worker_lifecycle.py` (RFC-009):
  ...
  - **R-10.5** (non-daemon interpreter-exit blocking under STOP_TIMEOUT / START_TIMEOUT) remains **Open — documented architectural trade-off** across `ThreadWorker` / `stop()` / `Agent.terminate` / `Agent.__deactivating` / `MqttBroker.start` docstrings and every WARNING log emitted by the timeout path (worker + broker stop + broker start layers). RFC-010's stop helper and RFC-011's startup helper are both `daemon=True` (so helpers alone don't block interpreter exit), but a worker thread waiting on `broker.start()` or `broker.stop()` still does.
