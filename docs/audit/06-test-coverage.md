...
Result as of 2026-08-02 (after RFC-012 implementation):

```
525 passed, 0 failed, 0 xfailed(strict-pass), 2 xfailed  in ~49s
```

Broken down (single collected run):
- RFC-012 suite (`tests/unit/test_mqtt_broker_publish_result.py`): **74 passed** in ~0.15 s
...
- MqttBroker reconnect / lifecycle / start / auth / callbacks / empty: **48 + 11 + 13 + 6 + 8 + 3 = 89 passed** (3 lifecycle tests refactored under RFC-012 to prime broker to RUNNING and provide RFC-012-shaped success MessageInfo)
- Agent publish suites (`test_agent_publish_errors.py` + `test_agent_publish_sync.py`): **46 + 27 = 73 passed** unchanged (use `FakeBroker`, not `MqttBroker` — RFC-012 does not affect them)
- All other suites: **97 passed, 2 xfailed** in ~12 s (unchanged)
